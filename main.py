...
def main():
    ...
    parser.add_argument("circuit_file", help="Path to the .cir circuit description file.")
    ...
    if args.input:
        for val in args.input:
            if "=" not in val:
                parser.error(f"Invalid input format: '{val}'. Expected 'SIGNAL=SEQUENCE'.")
            name, seq = val.split("=", 1)
            if not seq.isdigit() or not all(c in '01' for c in seq):
                parser.error(f"Input sequence for '{name}' must be a binary string (e.g., '1011').")
            inputs[name] = seq
